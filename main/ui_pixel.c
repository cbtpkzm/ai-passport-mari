...
static lv_obj_t *s_theme_screen;
static lv_obj_t *s_cloud_fill[3];
static lv_obj_t *s_ground;
static lv_obj_t *s_ground_edge;
static lv_obj_t *s_ground_dark[8];
static lv_obj_t *s_soil[8];
...
static void add_cloud(lv_obj_t *parent, int x, int y)
{
    ...
    s_cloud_fill[0] = block(parent, x + 5, y + 4, 35, 10, 0xFFFFFF);
    s_cloud_fill[1] = block(parent, x + 12, y, 10, 9, 0xFFFFFF);
    s_cloud_fill[2] = block(parent, x + 27, y + 1, 9, 8, 0xFFFFFF);
}
...
lv_obj_t *ui_pixel_screen_create(const char *title)
{
    ...
    add_cloud(scr, 188, 8);
    s_ground = block(scr, 0, 286, 240, 34, UI_GRASS);
    s_ground_edge = block(scr, 0, 286, 240, 4, 0xA7D93E);
    for (int i = 0, x = 0; x < 240; ++i, x += 30) {
        s_ground_dark[i] = block(scr, x, 312, 18, 8, UI_GRASS_DARK);
        s_soil[i] = block(scr, x + 18, 316, 12, 4, 0x75452E);
    }
...
void ui_pixel_screen_set_period(lv_obj_t *screen, ui_time_period_t period)
{
    static const ui_time_palette_t palettes[] = {
        [UI_TIME_DAWN] = { 0xE98D86, 0xFFE1CF, 0x577D4D, 0x83A85A, 0x385B42, 0x5C3B35 },
        [UI_TIME_DAY] = { UI_SKY, 0xFFFFFF, UI_GRASS, 0xA7D93E, UI_GRASS_DARK, 0x75452E },
        [UI_TIME_DUSK] = { 0x665A8D, 0xD8B7C3, 0x334D47, 0x577665, 0x223934, 0x49343B },
        [UI_TIME_NIGHT] = { 0x17263F, 0x71809A, 0x203D37, 0x315B4E, 0x142B28, 0x2C2730 },
    };
    if (screen != s_theme_screen || period > UI_TIME_NIGHT) return;
    const ui_time_palette_t *palette = &palettes[period];
    lv_obj_set_style_bg_color(screen, lv_color_hex(palette->sky), 0);
    for (int i = 0; i < 3; ++i) {
        lv_obj_set_style_bg_color(s_cloud_fill[i], lv_color_hex(palette->cloud), 0);
    }
    lv_obj_set_style_bg_color(s_ground, lv_color_hex(palette->ground), 0);
    lv_obj_set_style_bg_color(s_ground_edge, lv_color_hex(palette->edge), 0);
    for (int i = 0; i < 8; ++i) {
        lv_obj_set_style_bg_color(s_ground_dark[i], lv_color_hex(palette->dark), 0);
        lv_obj_set_style_bg_color(s_soil[i], lv_color_hex(palette->soil), 0);
    }
}
